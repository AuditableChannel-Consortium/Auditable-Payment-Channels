...
Client.prototype.createCommitment = function (opts) {
    ...
    this.commitmentHex = this.commitmentTx.extractTransaction().toHex()
    return this.commitmentHex
}
...
function assert(channel, tx, cnt) {
    const program = 'bin/assert'
    const channelParams = []
    for(let i = 0; i < channel.length / 2; i++) {
        channelParams.push(parseInt(channel.substring(i * 2, i * 2 + 2), 16))
    }
    const command = program + ' ' + channelParams.join(' ') + ' ' + tx + ' ' + cnt
    console.log('\033[91m', command, '\033[0m')
    return child.execSync(command)
}

Client.prototype.auditable = function (amount) {
    // 支付通道UTXO
    const MultiSigUTXO = this.getMultiSigUTXO()
    ...
    // 构造支付交易
    const paymentTx = new bitcoin.Psbt({network: this._network})
        .addInput({
            hash: MultiSigUTXO.txid,
            index: MultiSigUTXO.vout,
            nonWitnessUtxo:  Buffer.from(MultiSigUTXO.hex, 'hex'),
    ...
    // 支付人签名
    paymentTx.signInput(0, this._clientKeyPair)
    paymentTx.validateSignaturesOfInput(0, this._clientPubKey)
    return assert(this.commitmentHex, paymentTx.toHex(), this._cnt + 1)
}
